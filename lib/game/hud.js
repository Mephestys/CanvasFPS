...
MyHud = tpf.Hud.extend({
	...
	init: function( width, height, showControls ) {
		...
		this.healthIcon.setPosition(1, this.height-this.healthIcon.tileHeight-2);
	},
	...
	draw: function(player, weapon) {
		...
		if (weapon) {
			...
			if (weapon.ammoIcon && weapon.secondaryAmmoIcon) {
				weapon.ammoIcon.draw();
				this.font.draw(weapon.ammo, 125, this.height - this.font.height - 8, ig.Font.ALIGN.RIGHT);
		...
		this.font.draw( player.health, 24, this.height - this.font.height + 1, ig.Font.ALIGN.RIGHT );
