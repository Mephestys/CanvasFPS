...
EntityPlayer = tpf.Entity.extend({
	...
	armor: 0,
	maxArmor: 100,
	armorType: null, // can be 'LIGHT' or 'HEAVY'
	...
	ready: function() {
		...
		this.giveWeapon(WeaponPlasmaRifle, 32, 8);
	},
	...
	update: function() {
		...
		if ( 
			this.currentWeapon &&
			(ig.input.state('shoot') || ig.input.state('shoot2') || (!ig.ua.mobile && ig.input.state('click')) )
		) {
			...
			if (!this.currentWeapon.depleted()) {
				if (ig.input.state('shoot') || (!ig.ua.mobile && ig.input.state('click'))) {
					this.currentWeapon.trigger(sx, sy, this.angle);
				}
				if (ig.input.state('shoot2')) {
					this.currentWeapon.trigger2(sx, sy, this.angle);
				}
			} else {
		...
		if (dx != 0 || dy != 0) {
			...
			bobOffset += Math.sin(this.bob) * this.bobMoveHeight;
		} else {
			this.bob += ig.system.tick * this.bobSpeed * 16;
			bobOffset += Math.sin(this.bob) * this.bobIdleHeight;
		}
		
		//const bobOffset = Math.sin(this.bob) * this.bobMoveHeight;
		
		if ( this.currentWeapon ) {
			this.currentWeapon.bobOffset = Math.sin(this.bob+Math.PI/2) * this.bobMoveHeight * 2;
	...
	giveWeapon: function( weaponClass, ammo, secondaryAmmo ) {
		...
		for( var i = 0; i < this.weapons.length; i++ ) {
			...
			if( w instanceof weaponClass ) {
				index = i;
				w.giveAmmo(ammo);
				w.giveSecondaryAmmo(secondaryAmmo);
			}
		}
		
		// New weapon?
		if( index === -1 ) {
			this.weapons.push( new weaponClass(ammo, secondaryAmmo) );
	...
	giveAmmo: function( weaponClass, ammo ) {
		for (let weapon of this.weapons) {
			if (weapon instanceof weaponClass) {
				weapon.giveAmmo(ammo);
			}
		}
	},

	giveSecondaryAmmo: function(weaponClass, ammo) {
		for (let weapon of this.weapons) {
			if (weapon instanceof weaponClass) {
				weapon.giveSecondaryAmmo(ammo);
			}
		}
	},
	...
	switchWeapon: function( index ) {
		...
		if (this.currentWeapon.ammoIcon && !this.currentWeapon.secondaryAmmoIcon) {
			this.currentWeapon.ammoIcon.setPosition( 
				128, 
				ig.game.hud.height-this.currentWeapon.ammoIcon.tileHeight - 1
			);
		}
		...
		if (this.currentWeapon.ammoIcon && this.currentWeapon.secondaryAmmoIcon) {
			this.currentWeapon.ammoIcon.setPosition( 
				128, 
				ig.game.hud.height - this.currentWeapon.ammoIcon.tileHeight - 11
			);
			this.currentWeapon.secondaryAmmoIcon.setPosition(
				128,
				ig.game.hud.height - this.currentWeapon.secondaryAmmoIcon.tileHeight - 1
			);
		}
